...
% </ITA-Toolbox>
    ...
    properties
        ...
        r_mem       = []
        k           = 2 * pi / double(ita_constants('c')) .* ita_ANSI_center_frequencies;
        displacement  = [];
    end
    ...
    methods
        function this = itaSLAYER(varargin)
            % 1 argument    : Triggers the copy constructor.
            % 2 arguments   : Needed for a simulation based on real
        ...
        function value = get.velocitiesSH(this)
            ...
            for idf = 1:this.nFrequencies
                vel_list(1,:,idf) = 1i * this.k(idf) * c .* disp_vec;
            end
        ...
        function generate_Mall(this)
            ...
            if size([temp_unique_angles temp_transVec],2)==0
                ...
                D_mat_ele = zeros(this.nCoeff,this.nCoeff,size(really_unique_ele,1));
                % JCK: Adjust to changed 2nd unique() output argument. (column vector instead of row vector) 
                % Was D_mat_ele = zeros(this.nCoeff,this.nCoeff,size(really_unique_ele,2));
