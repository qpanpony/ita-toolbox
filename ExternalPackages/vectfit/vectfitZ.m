function [SER,poles,rmserr,fit,opts]=vectfitZ(f,s,poles,weight,opts)
%
...
if opts.skip_pole~=1
    ...
    else % generate orthonormal bases
        P = 1;
        for m = 1:N
            if cindex(m)==0      %real pole
                Dk(:,m)  = sqrt(1 - abs(LAMBD(m,m)))*P./(s-LAMBD(m,m));
            elseif cindex(m)==1  %complex pole, 1st part
                Dk(:,m)  = abs(1+LAMBD(m,m))*sqrt((1 - abs(LAMBD(m,m))^2)/2)*P.*(1 - s)./((s-LAMBD(m,m)) .* (s-LAMBD(m,m)'));
                Dk(:,m+1)= abs(1-LAMBD(m,m))*sqrt((1 - abs(LAMBD(m,m))^2)/2)*P.*(1 + s)./((s-LAMBD(m,m)) .* (s-LAMBD(m,m)'));
            end
            %         if cindex(m)==0      %real pole
            %             Dk(:,m)  = sqrt(2*real(LAMBD(m,m)))*P./(s-LAMBD(m,m));
            %         elseif cindex(m)==1  %complex pole, 1st part
            %             Dk(:,m)  = sqrt(2*real(LAMBD(m,m)))*P.*(s-abs(LAMBD(m,m)))./((s+LAMBD(m,m)) .* (s+LAMBD(m,m)'));
            %             Dk(:,m+1)= sqrt(2*real(LAMBD(m,m)))*P.*(s+abs(LAMBD(m,m)))./((s+LAMBD(m,m)) .* (s+LAMBD(m,m)'));
            %         end
            %         P = P .* (s-LAMBD(m,m)') ./ (s+LAMBD(m,m));
            P = P .* (1-LAMBD(m,m)'*s) ./ (s - LAMBD(m,m));
        end
    end
    ...
    roetter(abs(roetter) > 1) = 1./roetter(abs(roetter) > 1);
...
if opts.skip_res~=1
    ...
    fit=fit.'; f=f.';
    diff=bsxfun(@times,weight,fit-f);
