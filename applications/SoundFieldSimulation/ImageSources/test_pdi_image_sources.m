function isIR = test_pdi_image_sources(sPos, rPos, Z, Lx, Ly, Lz, c, rho0, t_max, sR, angleDependent, shiftToSample)
    ...
    for nx = -maxOrderX:1:maxOrderX
        for ny = -maxOrderY:1:maxOrderY
            for nz = -maxOrderZ:1:maxOrderZ
                ...
                d      = norm(rPos - coords); % distance
                t      = d / c; % traveling time
                ...
                if t <= t_max
                    ...
                    % richtcharakteristik, quellgröße mit r, theta, phi und
                    % entsprechender spielgelung muss hier hin % TODO %
%                     directionalfactor =  1;% ita_sph_base(itaCoordinates([0 0 0],'sph'),order)
                    
                    % % %                     hankel = ita_sph_besselh(order,2,k*d);
                    % % %                     p_magn = hankel * directionalfactor *...
