...
sArgs = struct('pos1_data','itaSuper','nodb',ita_preferences('nodb'),'figure_handle',[],'axes_handle',[],'linfreq',ita_preferences('linfreq'),'linewidth',ita_preferences('linewidth'),'fontname',ita_preferences('fontname'),'fontsize',ita_preferences('fontsize'), 'xlim',[],'ylim',[],'axis',[],'aspectratio',[],'hold','off','precise',true,'ylog',false,'unwrap',false,'stderr',false,'stdDev',[],'N',[]);
...
%% plot command
if ~sArgs.nodb
    lnh = errorbar(repmat(bin_vector,1,inputdata.nChannels),plotData,errPlotData1,errPlotData2,'lineWidth',sArgs.linewidth);
else
    lnh = boxplot(inputdata.freq.',inputdata.freqVector);
end
