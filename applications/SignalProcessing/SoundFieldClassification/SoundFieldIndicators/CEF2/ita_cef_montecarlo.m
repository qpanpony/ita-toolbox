%% Simulate many rooms a few times and have a look at the coherence
...
for HRTFid = 1:numel(HRTFFileNames);
    ...
    for idsim = 1:4000
        %% Room settings
        x = rand(1) * 5 * [5 4 3]; % Room dimensions
        T = rand(1)*2+0.1;
        drr = (rand(1)*30-15);
        r_in_rh = 1./(10.^((drr)./20));
        snr = rand(1) * 40 - 20;
        ...
        mkdir(roomtime);
        save([roomtime filesep 'MonteCarloSettings.mat'],'x','T','r','bs','V','S','r_in_rh','drr','HRTFFileName','uMode','snr')
