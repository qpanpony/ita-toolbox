...
if nargin == 1
    if isa(varargin{1},'itaMeshElements') && strfind(varargin{1}.type,'volume')
        volumeElements = varargin{1};
...
if length(volumeElements.nodes(1,:))==20|| length(volumeElements.nodes(1,:))==8
    edgeNodes = 1:2:size(list,2);
else
    edgeNodes = 1:size(list,2);
end
list_sort = sort(list(:,edgeNodes),2);
% list_sort = sort(list,2);
% sort out elements
l_list = size(list_sort,1);
h = waitbar(0,'Please wait... there are no surface elements - and a lot of work for me!');
...
while ~isempty(list)
    ...
    waitbar((l_list-size(list,1)) / l_list)
end
