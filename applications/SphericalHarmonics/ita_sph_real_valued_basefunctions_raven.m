function SH = ita_sph_real_valued_basefunctions_raven(sampling, nmax)
...
for n = 0:nmax
    ...
    SH(:, idxLeftSide)  = repmat(N(2:end) ,[sampling.nPoints 1]) .* P(:,2:end) .* sin(sampling.phi * (1:n));		%% !!!!!!! P(:,2:end) .* sin(-sampling.phi * (1:n)) in ITA-Toolbox!!!! No minus for RAVEN, just as in Zotters Diss
	%% ATTENTION!!!!!
end
end
