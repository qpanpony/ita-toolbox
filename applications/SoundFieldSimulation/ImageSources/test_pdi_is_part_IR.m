% function isIR = is_part_IR(sPos, rPos, Z, Lx, Ly, Lz, c, rho0, t_max, sR, angleDependent, shiftToSample)
% Original Author: Marc Aretz 
% Adaptation: Pascal Dietrich 2011
% Spiegelschallquellen Berechnung für Quaderraum

%% Abschätzungsformel - Baumgartner IEM, Graz, Bachelorarbeit S.14, Eq. 2.11
n  = 3;
f = [10 100 1000 10000]
kr = n*(n+1)/2
k = 2 * pi * f / 340;
f
r = kr ./ k % viel größer als dieser abstand, damit fernfeld bedingung erfüllt.
...
clear hankel
for order = 1
    hankel(order+1,:) = ita_sph_besselh(order,2,kd);
end
figure; semilogy(kd,[1./kd; abs(hankel)])
xlabel('kr')
...
profile on
tic
x = test_pdi_image_sources(sPos, rPos, Z, Lx, Ly, Lz, c, rho0, t_max, sR, angleDependent, shiftToSample);
toc
profile viewer
...
%% formula
Znorm = 10 % normalized impedance of the wall Z_wall/(rho0*c)
theta = (-90:0.1:90)/180*pi;
R = (Znorm .* cos(theta) -1 )  ./ (Znorm .* cos(theta) +1 );

figure
subplot(2,2,1)
plot(theta/pi*180,real(R))
ylim([-1 1])
title('real')
subplot(2,2,2)
plot(theta/pi*180,imag(R))
ylim([-1 1])
title('imag')
subplot(2,2,3)
plot(theta/pi*180,abs(R))
ylim([0 1])
title('abs')
subplot(2,2,4)
plot(theta/pi*180,angle(R))
ylim([-pi pi])
title('angle')
