function [SER,poles,signalerrorratio,fit,opts]=vectfit3(f,s,poles,weight,opts)
%
...
%********************************************************************************
%
% note: andrey modification or comment = §
...
weight = weight.'; % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
if length(weight(1,:)) == 1      % §Use the same weight for all functions of f.
    common_weight = 1;
elseif length(weight(1,:)) == Nc % §If f is vector of curves it is possible to choose different weights for each curve.
    common_weight = 0;
else
...
persistent hfig % use always the same plot
if opts.spy2
...
if opts.skip_pole ~= 1
    ...
    if opts.asymp == 1 || opts.asymp == 2
        Dk(:,N+1) = 1;      % §make d = 1 even if you choose opts.asymp = 1?
    elseif opts.asymp == 3
        Dk(:,N+1) = 1;
        Dk(:,N+2) = s;
    end
    ...
    if opts.relax == 1
        ...
        for n = 1:Nc
            ...
            for m = 1:N + offs %left block
                A(1:Ns,m) = weig .* Dk(1:Ns,m); % §Equation A.3 left block, defining A_k
            end
            inda = N + offs;
            for m = 1:N + 1 %right block
                A(1:Ns,inda+m) = -weig .* Dk(1:Ns,m) .* f(n,1:Ns).'; % §Equation A.3 right block, defining A_k
            end
            
            A = [real(A); imag(A)]; % §Eq. A.8, separate real and imaginary part
            ...
            [Q,R] = qr(A,0);                       % §Orthogonal-triangular decomposition (for faster calculation)
            ind1  = N + offs + 1;
            ind2  = N + offs + N + 1;
            R22   = R(ind1:ind2, ind1:ind2);       % §Residues from sigma function for the triangular matrix?
        ...
        x = AA\bb;                         % WRONG->§Calculate: residues from f; d ; h ; and residues from sigma
        % size(x),size(Escale)
        x = x .* Escale.';
    ...
    for n = 1:N
        m = m + 1;
        if m < N
            % Transform again the poles in complex notation (conjugate)
    ...
    ZER     = LAMBD - B * C.'/D; % §Equation B.1 from the paper
    roetter = eig(ZER).';        % §Calculate the new poles (zeros of sigma)
    ...
    %=============================================
    %Sorterer polene s.a. de reelle kommer først: % §Sorts poles S.A., the real comes first
    for n=1:N
        for m=n+1:N
    ...
    roetter = roetter - 2 * 1i * imag(roetter); %10.11.97 !!! % §change the signal of the imaginary part
    SERA    = roetter.';
...
%=========================================================================
%=========================================================================
flag_test = 0; %andrey: it was difined in a wrong place
if opts.skip_res ~= 1
    ...
    % §andrey: if common_weight == 1, Dk is calculated two times
...
%         X = abs(A) \ abs(BB);
    ...
    %     flag_test = 0;
    if isfield(opts,'s') && length(opts.s) ~= length(s)
        s         = opts.s; %     andrey: ??????????
        Ns        = length(opts.s);
        fit       = zeros(Nc,Ns);
        flag_test = 1;
    ...
    % andrey: §Calculate the fit, for the plot, with the same formula §
    Dk = zeros(Ns,N);
    for m = 1:N
        Dk(:,m) = 1 ./ (s-SERA(m)); 
    end
    ...
    if ~flag_test
        ...
        diff = fit - f;
        rmserr =   norm(abs(diff).* weight); % sqrt(sum(sum(abs(diff).^2.* weight))) / sqrt(Nc*Ns); % andrey: Weight is also a matrix with the same size of f. Calculate the energy of the error. Original line: rmserr = sqrt(sum(sum(abs(diff.^2)))) / sqrt(Nc*Ns);
        signalerrorratio = 20*log10(norm(abs(f)) / rmserr); % andrey: energy from the curve devided by the energy of the error. Result in dB.
            ...
            if opts.logx == 1
                ...
                if opts.phaseplot == 1
                    ...
                    h5 = plot(freq,180*unwrap(angle(fit))/pi,'r--'); hold off
                               ...
                               figure(hfig), subplot(nSubs,1,1)
         % andrey: don't need to plot every time again.
