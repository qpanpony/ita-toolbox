...
%         if chosenSignal
%             %B
%             changeColor('green', 'A', handles)
%             changeColor('cyan', 'B', handles)
%         else
%             %A
%             changeColor('cyan', 'A', handles)
%             changeColor('green', 'B', handles)
%         end
...
refSignal = handles.signals(config, 1);
signals = handles.signals(config, 2:end);
...
for idx = 1:handles.roundLength
    ...
    %testq=QuestMean(quest);
    %testq=QuestQuantile(quest,0.45);
    testq=QuestQuantile(quest,0.55);
    
    angleOfWall = round( testq );
    
%   limit angleOfWall
    if angleOfWall < 1
        angleOfWall = 1;
        disp('Wall angle is smaller than 1°')
    elseif angleOfWall > 30
        angleOfWall = 30;
        disp('Wall angle is bigger than 30°')
    end
...
%     correct = [];
    ...
    if correctAnswers(idx)
        signalA = refSignal;
        signalB = signals(angleOfWall);
        
        %The Different Signal is A
    else
        signalA = signals(angleOfWall);
        signalB = refSignal;
    end
...
%         end
    ...
    %Save Data in Result-Matrix
    result(idx,3) = correct;
    result(idx,1) = angleOfWall;
    
    %Update Quest
    quest = QuestUpdate(quest, angleOfWall, correct);
