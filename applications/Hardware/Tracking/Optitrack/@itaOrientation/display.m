...
% Display information about content of class object
...
if numel(this) > 1
    disp('multi instance class, TODO: implement this in itaOrientation.display');
    return;
end
...
elseif nPoints > 5000
    ...
    % maximum entry
    maxVal = max(max([abs(this.qw),abs(this.qx),abs(this.qy),abs(this.qz)]));
    if isnan(maxVal) || (maxVal == 0) || isempty(maxVal)
        maxVal = 1;
    end
    digitsBehind = 2;
    ...
    for iPoint = 1:nPoints
        fprintf(1, [inputname(1) '.n(' numberStyle  ') = ' prefix '  ' coordStyle '  ' coordStyle '  ' coordStyle '  ' coordStyle '\t]'], iPoint, this.mOrient(iPoint).e');
        if useAdditionalInput && nPoints>1
