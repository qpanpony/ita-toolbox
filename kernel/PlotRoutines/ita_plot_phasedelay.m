function varargout = ita_plot_phasedelay(varargin)
%ITA_PLOT_PHASEDELAY - plots the phase delay of a spectrum
%  This function plots the phase delay of a spectrum.
%
%  Syntax:
%   ita_plot_phasedelay(audioObjIn, options)
%
%   Options (default):
...
%           'normalize' (false) : 
%  Example:
%   figure_handle = ita_plot_phasedelay(audioObjIn)
%
%
%   Reference page in Help browser 
%        <a href="matlab:doc ita_plot_phasedelay">doc ita_plot_phasedelay</a>
...
if numel(data) > 1
    fgh = ita_plot_phasedelay(data(1), varargin{2:end});
    for idx = 2:numel(data)
        ita_plot_phasedelay(data(idx), varargin{2:end},'figure_handle',fgh,'hold','on');
    end
    return;
end
...
%% Get plot data
%get phase delay vector
plotData = ita_phasedelay(data);
...
% get nice limits, phase delay usually in fractions of a second
...
sArgs.xUnit      = 'Hz';
sArgs.yUnit      = 's';
sArgs.titleStr   = ['Phasedelay - ' data.comment];
sArgs.xLabel     = 'Frequency in Hz';
sArgs.yLabel     = 'Phasedelay in rad*s';
