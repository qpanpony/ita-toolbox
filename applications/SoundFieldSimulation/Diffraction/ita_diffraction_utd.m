...
function D = get_diffr_coeff( wedge, k, alpha_d, alpha_i, rho, r, theta_i, n )
    ...
    if any( singularities_present )
        [term1, term2, term3, term4] = approx_for_singularities();
    else
        term1 = Cot1 .* F1;
        term2 = Cot2 .* F2;
        term3 = Cot3 .* F3;
...
function [mask1, mask2, mask3, mask4] = check_for_singularities( wedge, alpha_i, alpha_d, n )
    ...
    mask1 = abs( ( alpha_d - alpha_i ) - 2 * pi * n * N_p( n, alpha_d - alpha_i ) + pi ) < eps;
    mask2 = abs( - ( alpha_d - alpha_i ) + 2 * pi * n * N_n( n, alpha_d - alpha_i ) + pi ) < eps;
    mask3 = abs( ( alpha_d + alpha_i ) - 2 * pi * n * N_p( n, alpha_d + alpha_i ) + pi ) < eps;
    mask4 = abs( - ( alpha_d + alpha_i ) + 2 * pi * n * N_n( n, alpha_d + alpha_i ) + pi ) < eps;
end

function [term1, term2, term3, term4] = approx_for_singularities( n, k, L, alpha_i, alpha_d )
