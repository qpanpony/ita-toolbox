function varargout = minMagnPhase(varargin)
% minimum value for magnitude and phase individually
...
if numel(result)>1 %get max over multiple instances and not over channel of each struct
    ...
    magnMin = squeeze(min(abs(data),[],3));
    ...
    % calculate min in magn and phase separately
    magnMin = squeeze(min(abs(result.freqData),[],2)); 
...
for idxCh = 1:result.nChannels   %alter name field of all channels
    resChannelNames{idxCh} = ['minMagnPhase(' resChannelNames{idxCh} ')'];
end
result.channelNames = resChannelNames;

%% Add history line
varargout{1} = ita_metainfo_add_historyline(result,'itaSuper.minMagnPhase',varargin);
end